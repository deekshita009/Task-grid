<html>
    ...
    <body>
                ...
                <form id="fwdform">
                    ...
                    <select name='dept' id="dept" required>
                        ...
                        <option value="CSBS">Computer Science and Business
                            System</option>
                        <option value="japanese_coordinator">Japanese </option>
                    </select>
                    <br><br>
                    <label for="fac_name">Choose faculty</label><br>
                    <select style="width:362px;" name="fac_name" id="fac_name"
                        required>
        ...
        <!--Complete task form-->
        <div class='mod' id='submod'>
            <div class="topdiv">Submit completed task
                <span class="close"
                    onclick="$('#submod').hide()">&times;</span>
            </div>
            <div style="padding:20px;">
                <form id="subform">
                    <input type="hidden" name="taskid" id="taskid">
                    <input type="hidden" name="taskassid" id="taskassid">

                    <label for="proof">Submit the proof of Completion</label>
                    <input type="file" name="proof" id="proof">
                    <br><br>
                    <label for='exp'>What is done?</label><br>
                    <input type="text" style="width:350px; height:70px;"
                        name="exp" id='exp' required>
                    <br><br>
                    <button type='submit' class="sub_mod_btn">Submit</button>
                </form>
            </div>
        </div>

        <!--Request deadline extension form-->
...
function loadtasks(table) {
    $.post('db/api.php', { action: 'readtasks' }, function(res) {
        ...
        res.forEach((r, i) => { // taken as list of lists,i is index and r is a row
            ...
            if (r.status == 'Assigned') {
                html = `<a class='status_btn' id='acc_btn' style='background-color: blue; color:white;' onclick="accepttask(${r.task_assignment_id})">Accept</a>`;
            }
            else if(r.status.toLowerCase() == 'completed'){
            ...
            else if(r.status == 'Pending' && r.action != 'Submitted'){
                html = `<a class='status_btn' style='background-color:orange;color:white;' onclick="showbtns(this)">${r.status}</a>
                <div class="btns">
                    <button onclick="opensubmodal(${r.task_assignment_id}, ${r.task_id})" class="cmp" title="Mark as Completed">✅</button>
                    <button class="req" onclick="reqdl(${r.task_assignment_id}, ${r.task_id})" title='deadline extension request'>🙋‍♂️</button>
                    <button class="fwd" onclick='fwd(${r.task_assignment_id}, ${r.task_id})' title = 'Forward task'>➡</button>
                </div>`;
            }
            else{
                html = `<a class='status_btn_inactive' onclick="showbtns(this)">${r.status}</a>`
            }
            table.row.add([
                i + 1,
                r.assigned_by,
...
//open modals for request deadline
function reqdl(taskassid, taskid){
    $('#reqmod').show();
    $('#reqform').data('taskid', taskid);
    $('#reqform').data('taskassid', taskassid);
}

//open modal for forwarding
function fwd(taskassid, taskid){
    $('#fwdmod').show();
    $('#fwdform').data('taskid', taskid);
    $('#fwdform').data('taskassid', taskassid);
}

//open modal for forwarding
function opensubmodal(taskassid, taskid){
    $('#submod').show();
    $('#taskid').val(taskid);
    $('#taskassid').val(taskassid);
}
//accept the task aasigned
function accepttask(taskassid){
    console.log("HI");
    $.post('db/api.php', {action:'accept_task', tid:taskassid}, function(res){
        Swal.fire({
                title: "Accept Task",
                text: "Task assigned is accepted", 
                icon: "info"
            });
    },"json");
    loadtasks(table);
...
$('#dept').on('change', function(){
    if($('#dept').val()!=''){
    $.post('db/api.php', {action:'filterfaculty', dept:$('#dept').val()}, function(res){
...
$('#fwdform').on('submit', function(e) {
    ...
    const taskassid = $(this).data('taskassid');
    const faculty = $('#fac_name').val();

    $.post('db/api.php', { action: 'forward', taskId: taskid, taskAssId: taskassid, name: faculty }, function(res) {
        console.log(res); 
...
$('#reqform').on('submit', function(e) {
    ...
    $.post('db/api.php', { action: 'requestdeadline', taskId: taskid, taskAssId: taskassid, Extdl : $('#deadline').val(), reason:  $('#reason').val() }, function(res) {
        console.log(res); 

        if (res.response === 'success') {
            Swal.fire({
                title: "Deadline Requested",
                text: "Request for deadline extension is submitted",
                icon: "info"
            });
        } else {
            Swal.fire({
...
$('#subform').on('submit', function(e){
    ...
    console.log("Hi");
    formdata.append("action", "submitproof");
    $.ajax({
        url:'db/api.php',
        type: 'POST',
        data: formdata,
...
function editstatus(id){   
    Swal.fire({
        ...
        if(result.isConfirmed){
            $.post('db/api.php', {action:'update_status', status:'Completed', task_assignment_id: id}, function(res){
