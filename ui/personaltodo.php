...
<head>
  ...
  <style>
    ...
    .day { width: 60px; height: 60px; margin: 5px; display: flex; justify-content: center; align-items: center; border-radius: 10px; cursor: pointer; background: #fff; border: 1px solid #ddd; transition: 0.2s; }
    .day:hover { background: #4e73df; color: #fff; transform: scale(1.05); }
    .day.today { background: #4e73df; color: #fff; font-weight: bold; }
    .day.selected { background: #6f42c1; color: white; }
    #calendarModal .modal-dialog { max-width: 900px; width: 90%; margin: 2% auto; }
    #calendarModal .modal-content { border-radius: 20px; box-shadow: 0 8px 40px rgba(0, 0, 0, 0.3); }
    #calendarModal .modal-header { background: linear-gradient(90deg, #4e73df, #6f42c1); color: #fff; }
    #addTaskModal .modal-dialog, #editTaskModal .modal-dialog { max-width: 700px; width: 90%; margin: 2% auto; }
    #addTaskModal .modal-content, #editTaskModal .modal-content { border-radius: 20px; box-shadow: 0 8px 40px rgba(0, 0, 0, 0.3); }
    #addTaskModal .modal-header, #editTaskModal .modal-header { background: linear-gradient(90deg, #4e73df, #6f42c1); color: #fff; }
    .modal-backdrop { z-index: 1050; }
    .modal { z-index: 1051; }
    #calendar { width: 100%; max-width: 520px; margin: 0 auto; }
    .weekday-row { display: grid; grid-template-columns: repeat(7, 60px); gap: 10px; justify-content: center; margin-bottom: 8px; user-select: none; }
    .weekday-label { text-align: center; font-weight: 600; color: #555; font-size: 0.85rem; }
    .day-grid { display: grid; grid-template-columns: repeat(7, 60px); gap: 10px; justify-content: center; }
    #calendar .day { margin: 0; width: 60px; height: 60px; display: flex; align-items: center; justify-content: center; position: relative; box-sizing: border-box; }
    #calendar .day.empty { visibility: hidden; background: transparent; border: none; }
    .day-number { font-size: 1rem; }
    #calendar .day.has-task::after { content: ""; width: 8px; height: 8px; background: #28a745; border-radius: 50%; position: absolute; bottom: 6px; right: 6px; box-shadow: 0 0 0 2px rgba(0, 0, 0, 0.05); }
    #calendar .day.today { outline: 2px solid rgba(78, 115, 223, 0.9); }
    #calendar .day.selected { box-shadow: inset 0 0 0 3px rgba(111, 66, 193, 0.12); background: #6f42c1 !important; color: #fff !important; }
  </style>
</head>
  ...
  <script>
    $(document).ready(function() {
      ...
      $(document).on("click", ".completeTask", function() {
        const date=$(this).data("date"); const index=$(this).data("index"); const tasks=JSON.parse(localStorage.getItem(tasksKey))||{};
        Swal.fire({title:'Are you sure?', text:"You won't be able to revert this!", icon:'warning', showCancelButton:true, confirmButtonColor:'#3085d6', cancelButtonColor:'#d33', confirmButtonText:'Yes, mark as completed!'})
        .then(result=>{if(result.isConfirmed){tasks[date][index].status="Completed"; localStorage.setItem(tasksKey, JSON.stringify(tasks)); loadTasks(selectedDate); Swal.fire("Completed!","Task marked as completed.","success");}});
      });

      $(document).on("click", ".editTask", function(){
        const date=$(this).data("date"); const index=$(this).data("index"); const tasks=JSON.parse(localStorage.getItem(tasksKey))||{}; const t=tasks[date][index];
        $("#editDate").val(date); $("#editIndex").val(index);
        $("#editTaskTitle").val(t.title); $("#editTaskDescription").val(t.description);
        $("#editTaskStartDate").val(t.startDate); $("#editTaskDeadline").val(t.deadline);
      ...
      $("#editTaskForm").submit(function(e){
        e.preventDefault();
        const date=$("#editDate").val(); const index=$("#editIndex").val(); const tasks=JSON.parse(localStorage.getItem(tasksKey))||{};
        tasks[date][index]={ title:$("#editTaskTitle").val(), description:$("#editTaskDescription").val(), startDate:$("#editTaskStartDate").val(), deadline:$("#editTaskDeadline").val(), status:$("#editTaskStatus").val() };
        localStorage.setItem(tasksKey, JSON.stringify(tasks));
      ...
      $(document).on("click", ".deleteTask", function(){
        ...
        .then(result=>{if(result.isConfirmed){tasks[date].splice(index,1); localStorage.setItem(tasksKey, JSON.stringify(tasks)); loadTasks(date); Swal.fire("Deleted!","Task deleted successfully.","success");}});
      });
