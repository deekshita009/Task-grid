        ...
        res.forEach((r, i) => { // taken as list of lists,i is index and r is a row
            ...
            if(r.status.toLowerCase() == 'completed'){
                ...
                <div class="btns">
                    ...
                    <button class="fwd" onclick='fwd(${r.task_assignment_id}, ${r.task_id})' title = 'Forward task'>➡️</button>
                </div>`;
            }
            else{
                ...
                <div class="btns">
                    ...
                    <button class="req" onclick="reqdl(${r.task_assignment_id}, ${r.task_id})" title='Request to extend deadline'>🙋‍♂️</button>
                    <button class="fwd" onclick='fwd(${r.task_assignment_id}, ${r.task_id})' title='Forward task'>➡️</button>
                </div>`;
            }
            table.row.add([
...
$('#fwdform').on('submit', function(e) {
    ...
    $.post('backend/api.php', { action: 'forward', taskId: taskid, taskAssId: taskassid, name: faculty }, function(res) {
        console.log(res); 
...
$('#reqform').on('submit', function(e) {
    ...
    $.post('backend/api.php', { action: 'requestdeadline', taskId: taskid, taskAssId: taskassid, Extdl : $('#deadline').val(), reason:  $('#reason').val() }, function(res) {
        console.log(res); 
